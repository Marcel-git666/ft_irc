...
void Server::sendChanMode(Client &sender, Channel *chan) {
  ...
  if (DEBUG)
    std::cout << BLUE << "sending mode of chan " << chan->getChName()
              << " to the client " << sender.getNickname() << ENDCOLOR
              << std::endl;
...
void Server::applyMode(Client &sender, Channel *chan,
  ...
  if (chan->clientIsOperator(sender.getFd())) {
    ...
    if (spasePos != std::string::npos) {
      ...
      if (DEBUG)
        std::cout << BLUE << "modestring :" << modestring << ENDCOLOR
                  << std::endl;
    } else
      modes = modestring;
    if (modes[0] == '+') {
      ...
      for (size_t i = 1; i < modes.length(); i++) {
        if (chan->getModestring().find(modes[i]) == std::string::npos ||
            modes[i] == 'l' || modes[i] == 'k') {
          // Ira: check if mode isn't in a string, for k and l its possible, if
          // client want to change args
