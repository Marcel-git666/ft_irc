...
void Server::connectToChannel(Client *client, const std::string &name,
  ...
  for (std::map<int, std::string>::iterator it = members.begin();
       it != members.end(); it++) { // Ira: we need to send info about member
    // joined to all of the channel members
...
void Server::joinChannel(Client *client, std::string &args) {
  ...
  std::string key = "";
  size_t commaPos = args.find(","); // Ira: client send a list of channels he
                                    // wants to join separated by commas
  ...
  if (!ch)
    ...
    if (ch->clientIsMember(sender.getFd())) {
      std::string msg = ":" + sender.getNickname() + "!" +
                        sender.getUsername() + "@localhost PRIVMSG " + args +
                        "\r\n";
      std::map<int, std::string> members = ch->getMembers();
      for (std::map<int, std::string>::iterator it = members.begin();
           it != members.end(); it++) {
...
void Server::inviteToChan(Client &sender, const std::string &args) {
  ...
  if (hashtagPos != std::string::npos) {
    std::string targetsStr =
        args.substr(0, hashtagPos - 1); // Ira: need to cut space and hashtag
    ...
    for (std::vector<std::string>::iterator it = targets.begin();
      ...
      if (FD == sender.getFd()) // Ira: avoiding to send invitation to himself
        continue;
      if (FD > 0) {
        if (!ch->clientIsMember(
                FD)) { // Ira: if client isn't a member of channel
          std::string message = ":" + sender.getNickname() + "!" +
                                sender.getUsername() + "@localhost INVITE " +
                                *it + " " + msg + "\r\n";
          if (DEBUG)
            std::cout << GREEN << "Sending message " << msg << " from "
                      << sender.getNickname() << " to "
                      << _clients[FD]->getNickname() << ENDCOLOR << std::endl;
...
void Server::kickOutOfChannel(Client &sender, const std::string &args) {
  ...
  if (!ch) { // check if channel exists
    ...
    if (ch->clientIsOperator(sender.getFd())) {
      ...
      std::string targetsStr =
          args.substr(spasePos + 1, colonPos - (spasePos + 1));
      if (DEBUG)
      ...
      for (std::vector<std::string>::iterator it = targets.begin();
        ...
        if (ch->clientIsMember(clientFdsearch(*it))) {
          std::string msg = ":" + sender.getNickname() + "!" +
                            sender.getUsername() + "@localhost KICK " +
                            chanName + " " + *it;
          if (!reason.empty())
            msg += " " + reason;
          msg += "\r\n";
          ...
          for (std::map<int, std::string>::iterator it = members.begin();
               it != members.end();
               it++) { // Ira: we need to send info about member kicked of the
            // channel members
            sendMsgToFd(it->first, msg);
          }
