...
bool Server::registerClient(Client &client) {
  ...
  if (client.setRegistered()) { // Ira: registered if all set, if not return
                                // false in next if condition, all messages
                                // about errors have been sent before
    std::string msg = ":server 001 " + client.getNickname() +
                      " :Welcome to the IRC Network, " + client.getNickname() +
                      "\r\n";
    sendMsgToClient(msg, client);
    // Ira: messages below isn't obligated, but I did it to be more like the
    ...
    // channelmodes = i  (invite-only); t  (topic restricted); k (key/password);
    ...
    if (DEBUG)
      std::cout << RED << "Client FD " << client.getFd()
                << " couldn't be registered" << ENDCOLOR << std::endl;
  }
  return (true);
}
